...
│  ├─ unlock.html
│  ├─ unlock.js
│  ├─ temp-allow-reminder.html
│  ├─ temp-allow-reminder.js
│  ├─ dashboard.html
│  └─ dashboard.js
├─ shared/
│  ├─ storage.js
...
| `windows` | 创建和管理临时放行提醒窗 | 用于在浏览器一侧展示倒计时提醒 |
...
5. 提交失败时的响应应包含 `incorrectIndexes`，供解锁页只高亮答错题目并触发一次摇晃反馈。
...
| 统计数据保留说明 | 固定提示 | 无 | 仅保留最近 7 天，超过一周自动清理 |
...
3. 临时放行结束后拦截自动恢复。
4. 临时放行期间提醒窗显示剩余时间，并在结束后自动关闭。
5. InPrivate 未启用时能给出明确提示。
6. 快速退出按钮在正式模式不出现。
